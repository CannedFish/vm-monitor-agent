...
def api_test():
    ...
    created_container = "config" + str(uuid.uuid4())
    r = requests.get('http://127.0.0.1:%s/api/get_object?user=%s&key=%s&tenant_name=%s&auth_url=%s&container_name=%s&object_name=%s&with_data=1' %(settings['port'],USER, KEY,TENANT_NAME,AUTH_URL, CONTAINER_NAME, OBJECT_NAME))
